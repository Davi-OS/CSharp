...
class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        // Criando a Lista
        FilaDinamica filaJogadores = new FilaDinamica();
        filaJogadores.preencheLista(time, n);

        //exibindo os Jogadores na FilaDinamica
        filaJogadores.Exibir();
    }
}

class FilaDinamica
{
    public Queue<Jogadores> filaJogadores = new Queue<Jogadores>();
    public int n;
    int numOperacoes;
    string instrucao = "";
    string linha;

    //Jogadores temporarios necessarios para realiar as operações de  Remoçãoo e inserção na fila
    ...
    public void preencheLista(Jogadores[] jogadoresIniciais, int qnt)
    {
        ...
        for (int i = 0; i < qnt; i++)
        {
            filaJogadores.Enqueue(jogadoresIniciais[i]);
        }
        ...
        for (int h = 0; h < numOperacoes; h++)
        {
            ...
            switch (instrucao)
            {
                case "I":
                    ...
                    filaJogadores.Enqueue(temp[contaJogadoresTemp]);
                    contaJogadoresTemp++;
                ...
                case "R":
                    temp[contaJogadoresTemp] = new Jogadores();
                    temp[contaJogadoresTemp] = filaJogadores.Dequeue();
                    contaJogadoresTemp++;
    ...
    //exibindo os jogadores da fila
    //Passaei os elementos do Queue para um array, assim imprimindo os elementos do topo a base da fila
    public void Exibir()
    {
        int i = 0;
        Jogadores[] resultadoPilha = new Jogadores[filaJogadores.Count];
        foreach (Jogadores item in filaJogadores)
        {
            resultadoPilha[i] = new Jogadores();
            resultadoPilha[i] = item;
            i++;
        }
        for (i = 0; i < resultadoPilha.Length; i++)
        {
